...
int shb,shl,shr,d,v=255,kr,kl,kf,kb;
int i=0,min,number;
int j=0, out_cnt = 0;
...
void fastStop()
    {
    motor(2*cmp, 2*cmp, 2*cmp, 2*cmp);
    }
void waitForBall()
    {  
    out_cnt = 0;
    while(min < 500 && out_cnt < 50) 
        {
        sensor();
        fastStop();  
        out_cnt++;
...
void out2()
    ...
    if(kaf.r || kaf.l || kaf.b || kaf.f)
        {   
        // Out Lef-Back
        if(shl > 250 && shr < 100 && shb > 300 && number >= 6 && number <= 14)
            {
            moveForSeconds(1, 2);
            waitForBall();
            }
        // Out Right-Back
        if(shr > 250 && shl < 100 && shb > 300 && number >= 2 && number <= 10)
            {
            moveForSeconds(15, 2);  
            waitForBall();
            }
        // Out Right
        if(shl < 100 && shr > 300 && number > 8)
            {
            moveForSeconds(12, 2);
            waitForBall();
            }
        // Out Left
        if(shr < 100 && shl > 300 && number < 8)
            {
            moveForSeconds(4, 2);
            waitForBall();            
            }
        // Out Right-Front
        if(shl < 100 && shr > 250 && shb < 300 && (number <= 6 || number >= 14))
            {
            moveForSeconds(10, 2);
            waitForBall();
            }
        // Out left-Front
        if(shr < 100 && shl > 250 && shb < 300 && (number <= 2 || number >= 10))
            {
            moveForSeconds(6, 2);
            waitForBall();            
            }
